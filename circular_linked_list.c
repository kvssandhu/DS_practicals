...
void insert_at_beginning( int college_id, char college_name[100] )
{
    ...
    new_node = (struct node *) malloc (sizeof(struct node));    //create a new_node
    new_node -> college_id = college_id;    //insert college id into that new node
    strcpy(new_node -> college_name,college_name);      //insert college name into new node
    new_node -> next = start->next;     //next of new node will point to start
    node->next = new_node;      // start's next will point to new node ( since we are inserting at beginning )
    start = new_node;
